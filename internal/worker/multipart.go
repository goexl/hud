...
func (m *Multipart) Do() (err error) {
	if parts, pe := m.upload.Parts(m.self); nil != pe {
		...
		m.parts = make([]*internal.Part, 0, parts)
...
func (m *Multipart) uploads(urls []*internal.Url, parts int, err *error) {
...
func (m *Multipart) part(url *internal.Url, part int, wg *sync.WaitGroup, err *error) {
	defer wg.Done()
...
func (m *Multipart) send(url *internal.Url, bytes []byte) (rsp *resty.Response, err error) {
